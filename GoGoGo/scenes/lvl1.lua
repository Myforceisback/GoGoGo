...
function scene1:show(event)
    local sceneGroup = self.view;
     ...
     --Музыка lvl1
     local playSound = audio.play(Lvl1Music, {channel = 1, loops = -1});
    ...
    G = timer.performWithDelay(1000,yes,-1);
    ----

    --Обработчик столкновений с машинами

    local destroyLine = display.newRect(0, 800, 600, 1)
    physics.addBody(destroyLine,"static");
    ...
    local function crashLine(self,event)
        local collideObject = event.other
        if (collideObject.surfaceType == "dada") then
            -- timer.cancelAll()
            -- physics.stop()
    ...
    destroyLine.collision = crashLine
    destroyLine:addEventListener("collision")
    ----
